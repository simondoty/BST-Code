...
public class BSTJava<E extends Comparable<? super E> > {
    ...
    private BSTNode<E> recursiveAdd(BSTNode<E> node, E value) {
        ...
        if (comparison > 0)
            node.setRight(recursvieAdd(n.getRight(), value));
        if (comparison < 0)
            node.setLeft(recursiveAdd(n.getLeft(), value));
        return node;
    }
    ...
    private BSTNode<E> recursiveRemove(BSTNode<E> node, E value) {
        ...
        if (comparison > 0) 
            n.setRight(recursiveRemove(node.getRight(), vlaue));
        if (comparison < 0)
            n.setLeft(recursiveRemove(node.getLeft(), value));
        else { // comparison == 0
            size--;
            if (node.getRight() == null && node.getLeft() == null)
                n = null;
            else if (node.getRight() == null) // now that left is not null
                n = n.getLeft(); // return the left node and bypass current node
            else if (node.getLeft() == null)
                n = n.getRight();
            else {
                node.setData(max(n.getLeft());
